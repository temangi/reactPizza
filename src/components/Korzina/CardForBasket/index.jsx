...
const CardForBasket = ({basket,deletePizza,price}) => {
  const [mainPrice , setMainPrice] = useState(price)
  const [counterPizzas,setCounterPizzas] = useState(basket.length)

  const changePizzaPrice = (pizzaPrice) => {
    setMainPrice(mainPrice+pizzaPrice)
  }

  const pizzasBasket = () => {
    return basket.map((el,index) => <CardPizza key={index} {...el} changePizzaPrice={changePizzaPrice}  deletePizza={deletePizza}/>);
  };
      ...
      <article className={scss.container}>
        ...
        <section className={scss.cart__bottom}>
          <div className={scss.cart__bottom_details}>
            <span>
              Всего пицц: <b>{counterPizzas} шт</b>
            </span>
            <span>
              Сумма заказа: {mainPrice} <b className={scss.price}></b>
            </span>
          </div>
